...
class StudentCreateForm
  ...
  def initialize(existing_student = nil)
    ...
    @controller = StudentCreateFormController.new(self, existing_student)
  end
    ...
    @root_container = window('Универ', 300, 200) {
      ...
      vertical_box {
        @student_form = form {
          ...
          fields = [[:surname, 'Фамилия', false], [:first_name, 'Имя', false], [:middle_name, 'Отчество', false],
                    [:git, 'Гит', true], [:telegram, 'Телеграм', true], [:mail, 'Почта', true], [:phone_number, 'Телефон', true]]
  ...
  def validate_and_save
    ...
    begin
      stud = Student.new(
        first_name: @student_form.children[0].text.force_encoding("utf-8"),
        middle_name: @student_form.children[1].text.force_encoding("utf-8"),
        surname: @student_form.children[2].text.force_encoding("utf-8"),
        git: input_git,
        telegram: input_tg,
        mail: input_email,
      ...
      script = "display dialog \"#{e.message}\" with title \"Ошибка\""
